...
export default class UploadFiles extends Component {
	...
	componentDidMount = async () => {
	    ...
	    if (postid) {
	        const response = await api.get(`/filePost/post?postid=${postid}`);
	        ...
	        if (image !== undefined) {
	            const uploadedImage = image.map(file => ({
	                ...
	                url:
						...
						`/filePost/image?filename=${file.filename}`,
	                preview:
						api.defaults.baseURL +
						`/filePost/image?filename=${file.filename}`
	            }));
	        ...
	        if (video !== undefined) {
	            const uploadedVideo = video.map(file => ({
	                ...
	                url:
						api.defaults.baseURL +
						`/filePost/video?filename=${file.filename}`,
	                preview: yt
	            }));
	...
	verifyMimetype = (mimetype = '', filename = '') => {
	    ...
	    if (mimetype.split('/')[0] === 'image') {
	        return (
	            api.defaults.baseURL + '/filePost/image?filename=' + filename
	        );
	    } else {
	        return (
	            api.defaults.baseURL + '/filePost/video?filename=' + filename
	        );
	    }
	};
	...
	handleDelete = async (idFake, id) => {
	    ...
	    await api.delete(`/filePost/?idfile=${id}`);
	...
	processUpload = uploadFile => {
	    ...
	    api.post(`/filePost?postid=${this.props.postid}`, data, config)
	        .then(response => {
